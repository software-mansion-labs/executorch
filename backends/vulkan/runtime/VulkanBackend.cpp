...
class VulkanBackend final : public ::executorch::runtime::BackendInterface {
 public:
  ...
  Error execute(
    ...
    // `args` carries the delegate call's inputs followed by its outputs. If
    // the serialized graph disagrees about either count, the `args.size() -
    ...
    for (size_t i = 0; i < num_inputs; i++) {
      ...
      if (compute_graph->val_is_tensor(iref)) {
        ...
        // Mirror of output path: caller's SymInt input may arrive as a
        // scalar tensor (legacy convention) or as a plain Int EValue
        // (default when the Method's input schema declares SymInt).
        bool was_updated = false;
        if (args[i]->isTensor()) {
    ...
    for (size_t i = 0; i < num_outputs; i++) {
      ...
      if (compute_graph->val_is_tensor(oref)) {
        ...
        // Mirror of input path: write the graph's current SymInt value
        // into the caller's slot. The caller's EValue may be either a
        // scalar tensor (legacy convention) or a plain Int EValue
        // (default when the Method's output schema is SymInt).
        ...
        if (args[o]->isTensor()) {
          ...
          // Overwrite the EValue's int payload in place. EValue stores
          // its int as int64_t, so a SymInt fits without truncation.
