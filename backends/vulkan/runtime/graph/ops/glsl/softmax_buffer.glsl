...
// Supplied by the dispatch so it can scale with the row length.
...
void main() {
  ...
  // Combine the partials as a tree: the serial walk below ran in EVERY thread,
  // so its cost grew with the worker count. barrier() sits outside the guard.
