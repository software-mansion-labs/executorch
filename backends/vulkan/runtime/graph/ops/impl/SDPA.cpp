...
vkapi::ShaderInfo pick_sdpa_compute_attn_weights_shader_impl(
  ...
  // <base>_<variant>_<storage>_<storage>_<dtype>. This matches the gemma-fork
  // picker's convention so the audio path can route through these
  // (multi-row-correct) shaders.
  if (has_mask) {
    if (mask_per_head) {
...
void add_sdpa_compute_attn_weights_node(
  ...
  // Push constants for HAS_SOFTCAP variant: pair of floats (softcap,
  // inv_softcap). Inactive variants ignore them (block is not declared when
  // HAS_SOFTCAP=0).
  ...
  if (has_softcap) {
    push_constants.push_back(PushConstantDataInfo(&pcs[0], sizeof(float) * 2));
  }
...
void add_sdpa_compute_out_node(
  ...
  std::function<vkapi::ShaderInfo(
      ...
      pick_shader_fn;
  if (force_tiled) {
    pick_shader_fn = [](ComputeGraph* g,
                        const std::vector<ArgGroup>& a,
                        const std::vector<ValueRef>& r) -> vkapi::ShaderInfo {
      return pick_sdpa_compute_out_shader_impl(g, a, r, /*force_tiled=*/true);
    };
  } else {
...
void sdpa_impl(ComputeGraph& graph, const std::vector<ValueRef>& args) {
  ...
  if (fuse_softmax) {
    add_sdpa_compute_attn_weights_with_softmax_node(
        graph, q_projected, k_cache, input_pos_symint, attn_weights_softmax);
  } else {
