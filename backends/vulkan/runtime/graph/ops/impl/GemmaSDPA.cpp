...
std::unordered_map<const void*, ValueRef>& cache_map_for(ComputeGraph& graph) {
  static std::unordered_map<
      ComputeGraph*, std::unordered_map<const void*, ValueRef>> per_graph;
  return per_graph[&graph];
}
ValueRef realize_cache_once(
    ComputeGraph& graph, const ValueRef cache_data,
  ...
  const ValueRef cache = graph.add_tensor_like(
      cache_data, storage, utils::kWidthPacked);
  m.emplace(key, cache);
  return cache;
}
} // namespace
...
// (which has a multi-row corruption bug at row 1+) and onto the multi-row-correct
// Llama shaders.
...
void add_gemma_compute_attn_weights_node(
  ...
  GemmaShaderPicker picker{has_mask, /*mask_per_head=*/false,
                           /*has_softcap=*/false};
...
void gemma_sdpa_with_kv_cache_impl(
  ...
  const ValueRef k_cache = graph.add_tensor_like(
      k_cache_data, cache_storage, utils::kWidthPacked);
  const ValueRef v_cache = graph.add_tensor_like(
      v_cache_data, cache_storage, utils::kWidthPacked);
  ...
  gemma_sdpa_core(
      graph, q_projected, k_cache, v_cache, input_pos_symint, attn_mask,
      is_causal, out, softcap_value);
}
...
void gemma_custom_sdpa_impl(
  ...
  utils::StorageType cache_storage = graph.storage_type_of(q_projected);
  const ValueRef k_cache = realize_cache_once(graph, k_cache_data, cache_storage);
  const ValueRef v_cache = realize_cache_once(graph, v_cache_data, cache_storage);
