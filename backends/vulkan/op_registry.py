...
def register_gemma_sdpa_with_kv_cache_op():
    ...
    # never splits the cache write from the SDPA. See
    # PLAN_gemma4_custom_sdpa_shader.md §6 / §8 risk #4.
    return OpFeatures(
        inputs_storage=utils.CONTIGUOUS_ANY,
        supports_resize=True,
        supports_prepacking=True,
...
# Fused RMSNorm: one Vulkan dispatch replaces the
# (x*x → mean → +eps → rsqrt → mul → mul) chain. Buffer-only kernel; the
# C++ impl prepacks the weight tensor itself.
@update_features(
    [
